/**
 * Checkbox — drop-in port off styled-components.
 *
 * Still Radix Checkbox + controllable state + composed refs. The styled
 * rules (sizing, border, focus, checked/indeterminate/disabled colors, 44px
 * touch target via ::before) moved to `componentPolish.css` keyed on
 * `[data-strapi-checkbox-root]` + `[data-strapi-checkbox-indicator]`.
 *
 * useTheme() lookup for the check-icon fill replaced with a direct CSS var
 * reference — the icon receives `fill="var(--strapi-color-neutral0)"`
 * inline.
 */
...
const CheckIcon = ({ fill, ...props }: BoxProps<'svg'>) => {
  return (
    <Box
      ...
      fill={fill ? `var(--strapi-color-${fill})` : undefined}
      {...props}
    >
  ...
  ({ defaultChecked, checked: checkedProp, onCheckedChange, ...props }, forwardedRef) => {
    ...
    return (
      <BaseCheckbox.Root
        ...
        data-strapi-checkbox-root=""
        {...props}
      >
        <BaseCheckbox.Indicator forceMount data-strapi-checkbox-indicator="">
